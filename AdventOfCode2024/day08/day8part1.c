                ...
                for (y2 = 0; y2 < NUM_LINES; y2++)
                {
                    for (x2 = 0; x2 < strlen(lines[y2]); x2++)
                    {
                        ...
                        if (lines[y][x] == lines[y2][x2])
                        {
                            ...
                            if (x2 + x_difference < 0 || x2 + x_difference >= strlen(lines[y]))
                            {
                                continue;
                            }
                            if (y2 + y_difference < 0 || y2 + y_difference >= NUM_LINES)
                            {
                                continue;
                            }

                            if (antinodes[y2+y_difference][x2+x_difference] != '#')
                            {
                                antinodes[y2+y_difference][x2+x_difference] = '#';
                                count++;
                            }
                        }
                    }
