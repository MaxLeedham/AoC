...
int reach_target(long unsigned *numbers, int n, long unsigned target)
{
    ...
    new_numbers[1] = (long unsigned) (numbers[0] * (long unsigned) pow(10, (((int) log10(numbers[1])) + 1))) + numbers[1];

    concatenate = reach_target(&(new_numbers[1]), n-1, target);
